...
class AdminMenu:
        ...
        try:
            with open('student.data', 'r') as file:
                ...
                if not students_data:
                    print(BOLD + MAGENTA +" <Nothing to display> No student data found.")
                else:
                    for student in students_data:
                        print(f"{student['first_name']} {student['last_name']} :: {student['id']} --> {student['email']}" + RESET)
        ...
        try:
            with open('student.data', 'r') as file:
                ...
                if not students_data:
                    ...
                    grade_to_view = input(CYAN + "Enter the grade to view (HD/D/C/P/F): ").upper()
                            ...
                            for value in grades.values():
                                if isinstance(value, int):
                                    ...
                                    if student_grade == grade_to_view:
                                        print(f"{grade_to_view} --> [ {student['first_name']} {student['last_name']} :: {student['id']} --> GRADE: {grade_to_view} - Mark : {value} ]")
                                        found_students = True
            ...
            with open('student.data', 'r') as file:
                ...
                if not students_data:
                    ...
                    if pass_students:
                        ...
                        print(BOLD + BLUE +"PASS --> " + RESET + "[" + f"{pass_students_string}" + "]")
                        print()

                    if fail_students:
                        fail_students_string = ', '.join(fail_students)
                        print(BOLD + MAGENTA + "FAIL --> " + RESET + "[" + f"{fail_students_string}" + "]")
    ...
    def remove_student(self):
        ...
        try:
            ...
            student_id_to_remove = input(CYAN + "Enter the student ID to remove: " + RESET)
            updated_students_data = [student for student in students_data if student.get("id") != student_id_to_remove]
            ...
            if len(updated_students_data) < len(students_data):
                ...
                print("\nStudent with ID" + BOLD + YELLOW + f"{student_id_to_remove}" + RESET + "removed successfully.")
            else:
                print(MAGENTA + "\nNo student found with ID" + BOLD + YELLOW + f" {student_id_to_remove} "+ RESET)
    ...
    def clear_data(self):
        ...
        print()
        try:
            user_input = input(BOLD + RED + "Are you sure you want to clear the database? ( Y / N ): " + RESET).strip().lower()
